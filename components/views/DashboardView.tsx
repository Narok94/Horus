...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div className="w-full h-full max-h-full flex flex-col justify-between bg-transparent text-white font-sans antialiased overflow-hidden select-none relative pt-12 sm:pt-14 pb-1">
      
      {/* 1. COCKPIT HEADER COMPACTO */}
      <header className="flex justify-between items-center shrink-0 py-1.5 px-1 border-b border-white/[0.015]">
        ...
        <div className="flex-1 min-h-[160px] relative overflow-hidden group bg-gradient-to-b from-[#0F1014] to-[#040405] border border-white/[0.035] rounded-xl flex flex-col justify-between p-4.5 shadow-xl transition-all duration-300 hover:border-accent/10">
            ...
            {nextWorkout ? (
              <div className="space-y-2 flex-1 min-h-0 flex flex-col justify-between">
                ...
                {nextWorkout.exercises && nextWorkout.exercises.length > 0 && (
                  <div className="flex flex-col gap-1.5 py-1.5 select-none leading-none max-h-[140px] overflow-y-auto no-scrollbar pr-0.5 w-full">
                    {nextWorkout.exercises.map((ex, index) => (
                      <div 
                        key={ex.id || index}
                        className="flex items-center gap-2.5 px-3 py-1.5 rounded-lg bg-white/[0.015] border border-white/[0.04] shadow-[0_1px_2px_rgba(0,0,0,0.1)] hover:bg-white/[0.03] transition-all focus:outline-none w-full"
                      >
                        ...
                        <div className="flex flex-row items-center justify-between flex-1 min-w-0 pr-1">
                          <span className="text-[10px] sm:text-xs font-black uppercase text-zinc-105 text-wrap leading-tight text-left">
                            {ex.name}
                          </span>
                          <span className="text-[9.5px] sm:text-xs font-mono text-zinc-200 font-extrabold shrink-0 pl-2">
                            {ex.sets}x{ex.reps}
                          </span>
                        </div>
                      </div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 select-none leading-none">
                  <span className="bg-[#111318]/60 text-zinc-400 text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded border border-white/5">
                    {nextWorkout.exercises?.length || 0} Exer.
                  </span>
                  <span className="bg-[#111318]/60 text-zinc-400 text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded border border-white/5">
                    Foco: {nextWorkout.exercises?.[0]?.muscleGroup || 'Multijoint'}
                  </span>
                  <span className="bg-[#111318]/60 text-zinc-400 text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded border border-white/5 font-mono">
                    ~45 MIN
                  </span>
                </div>
              </div>
          ...
          <div className="relative z-10 flex gap-2.5 pt-2 shrink-0 select-none">
            ...
            <button
              ...
              className="flex-1 bg-transparent border border-white/10 hover:border-white/25 text-zinc-200 hover:text-white font-[950] uppercase text-[10.5px] py-3 rounded-lg hover:bg-white/[0.015] active:scale-[0.98] transition-all cursor-pointer tracking-wider text-center flex items-center justify-center gap-1.5"
            >
              Fichas
            </button>
          </div>
